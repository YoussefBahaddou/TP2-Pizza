...
import com.emsi.pizzaapplication.adapter.PizzaAdapter;
...
public class ProduitListActivity extends AppCompatActivity {
    ...
    private static final long LONG_PRESS_TIMEOUT = 2000;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        liste.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (!isLongPress) {
                    TextView idf = view.findViewById(R.id.id);
                    Intent intent = new Intent(ProduitListActivity.this, DetailsPizzaActivity.class);
                    intent.putExtra("idp", idf.getText().toString());
        ...
        liste.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                final int position = liste.pointToPosition((int) event.getX(), (int) event.getY());
                if (position >= 0) {
                    final View itemView = liste.getChildAt(position - liste.getFirstVisiblePosition());
    ...
    private void handleLongPress(View view) {
        final TextView idf = view.findViewById(R.id.id);
        TextView nom = view.findViewById(R.id.nom);

        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(this);
        alertDialogBuilder.setTitle("Delete Pizza");
        
        // Set a custom icon for the dialog
        alertDialogBuilder.setIcon(android.R.drawable.ic_dialog_alert);

        String message = "Are you sure you want to delete <b>" + nom.getText().toString() + "</b>?";
        alertDialogBuilder.setMessage(android.text.Html.fromHtml(message));

        alertDialogBuilder.setPositiveButton("Delete", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                ps.delete(ps.findById(Integer.parseInt(idf.getText().toString())));
                liste.setAdapter(new PizzaAdapter(ProduitListActivity.this, ps.findAll()));
                Toast.makeText(ProduitListActivity.this, "Pizza deleted", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        alertDialog.setOnShowListener(new DialogInterface.OnShowListener() {
            ...
            public void onShow(DialogInterface dialogInterface) {
                ...
                if (titleView != null) {
                    titleView.setTextColor(Color.parseColor("#D32F2F")); // Red color
                }

                Button positiveButton = alertDialog.getButton(DialogInterface.BUTTON_POSITIVE);
                positiveButton.setTextColor(Color.parseColor("#D32F2F")); // Red color
                positiveButton.setTypeface(null, Typeface.BOLD);

                Button negativeButton = alertDialog.getButton(DialogInterface.BUTTON_NEGATIVE);
                negativeButton.setTextColor(Color.parseColor("#757575")); // Gray color
            }
        });
